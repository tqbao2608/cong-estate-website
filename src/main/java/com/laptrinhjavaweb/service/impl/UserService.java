...
public class UserService implements IUserService {
    ...
    public List<BuildingManagerResponseDTO> findStaffByBuildingId(long id) {
        ...
        BuildingEntity buildingEntityList = buildingRepository.findById(id);
        // nguyên bản assignment -> dang can staff_id
        List<AssignmentBuildingEntity> assignmentBuildingEntityList = buildingEntityList.getAssignmentBuildingEntities();
        //List<UserEntity> assignmentStaffs = userRepositoryCustom.findByBuildingId(id);

        // list staff assignment Building
        List<Long> list = convertToListIds(assignmentBuildingEntityList);

        for (UserEntity item : userEntities) {

            BuildingManagerResponseDTO model = new BuildingManagerResponseDTO();
            model.setId(item.getId());
            model.setFullName((item.getFullName()));
            if (list.contains(item.getId())) {
                model.setChecked(SystemConstant.CHECKED);
            }
            results.add(model);
        }
        return results;
    ...
    private List<Long> convertToListIds(List<AssignmentBuildingEntity> assignmentStaffs) {
        List<Long> list = new ArrayList<>();
        for (AssignmentBuildingEntity item : assignmentStaffs) {
            list.add(item.getUserEntity().getId());
        }
        return list;
    }
}
